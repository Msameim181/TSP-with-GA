...
def print_function(data):
    for i in range(len(data)):
        print(data[i])
# Check if exist in array with no dublicated 
...
# Cities Coordinates:
def getCoordinates():
    Coordinates_file = (r"D:\Mahdi\University\Terms\Term 7\C.I - Aminian\Projects\Ex.9 - TSP\Cities.xlsx")
    Coordinates = []
    excel = xlrd.open_workbook(Coordinates_file) 
    sheet = excel.sheet_by_index(0)
    for i in range(sheet.nrows): 
            City_x = int(sheet.cell_value(i, 0))
            City_y = int(sheet.cell_value(i, 1))
            City_xy = [City_x, City_y]
            Coordinates.append(City_xy)
    return Coordinates
# ----------------------------------------------------------- #
...
# ----------------------------------------------------------- #
# # Evaluation :
# Calculate distance of two cities : Sqrt((X1 - X2)^2 + (Y1 - Y2)^2)
...
def totalCost_Fitness_Calculator(Path, Coordinates, NumberOfCities):
    ...
    City2_name = Path[0]
    City_one = Coordinates[City1_name]
    ity_two = Coordinates[City2_name]
    Distance = dist(City_one, City_two)
    AllDistance.append(Distance)
...
# ----------------------------------------------------------- #
# Show Outpot :
def outpot(FirstPopulation, NewPopulation, New_FullEvaluation, Epok):
    print("\n---------1-----------\n")
...
def TSP(Value, Persent):
    ...
    # ----------------------------------------------------------- #
    # **** Start While Loop
    Counter = 5000      # Maximume Loop and generation
    Epok = 0            # Epok counter
    ...
    while Counter > 0:
        ...
        ConditionFlag2, New_MinDist, New_MinFitness= stopCondition2(Population, CitiesCoordinates, NumberOfCities, Value, MinDist)
        # ----------------------------------------------------------- #
        # 7.1 Check Condition :
    ...
    # ----------------------------------------------------------- #
    # Output
    outpot(FirstPopulation, NewPopulation, New_FullEvaluation, Epok)
    # ----------------------------------------------------------- #
    # Plot :
...
# Run TSP : (Number Of Population, Persent of marge)
TSP(80, 60)
